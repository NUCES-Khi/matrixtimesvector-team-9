...
void matrix_vector_multiply_tiled(double **matrix, double *vector, double *result, int rows, int cols) {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < cols; j += TILE_SIZE) {
            for (int k = 0; k < TILE_SIZE; k++) {
                result[i] += matrix[i][j + k] * vector[j + k];
            }
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    matrix_vector_multiply_tiled(matrix, vector, result, rows, cols);
